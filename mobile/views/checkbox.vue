  ...
  <ul class="m-checkbox">
    <li v-for="item in listData" :key="item.id">
      ...
      <me-checkbox-group v-model="item.value" @on-change="onChange" :direction="item.direction" v-if="item.list.length>1">
        <me-checkbox v-for="it in item.list" :key="it.id" :name="it.id" :shape="it.shape" :icon="it.icon" :icon-size="it.iconSize" :icon-select="it.iconSelect" :checked-color="it.checkedColor" :disabled="it.disabled">{{it.label}}</me-checkbox>
      </me-checkbox-group>
      <template v-else>
