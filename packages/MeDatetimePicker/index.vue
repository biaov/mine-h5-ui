...
export default {
  ...
  methods: {
    ...
    setValue(tarDir, i) {
      ...
      if (i === 2 && show.includes(3)) {
        ...
        if (current === 2) {
           ...
           * 是否是闰月
           */
          listData[2].list = IsLeapyear(Year) ? 29 : 28
        } else {
          /**
       ...
       * 设置 currentValue
       */
      currentValue[show.indexOf(i)] = current
    },
    /**
     * 当日期值改变时，修改相应的状态
     */
    setDateDist(i) {
       ...
       * 实际移动的距离
       */
      let oldDist = distance[index]
      /**
       * 获取天个数
       */
      const len = listData[2].list - 1
      /**
       ...
       * 设置理想移动的距离
       */
      this.$set(distance, index, oldDist)
      /**
       * 开启过渡效果
       */
       ...
       * 设置 currentValue
       */
      currentValue[show.indexOf(3)] = count + 1
    },
    /**
     * 开始过渡
     */
    ...
    onTouchstart(e, i) {
       ...
       * 记录开始距离
       */
      this.startDistence = this.distance[show.indexOf(i)]
      /**
       * 记录开始的时间
       */
    ...
    onTouchmove(e, i) {
       ...
       * 设置实际移动的距离
       */
      this.$set(distance, show.indexOf(i), actualDist)
    },
    /**
     * 触摸结束
     */
    ...
    onMousedown(e, i) {
      ...
      document.onmouseup = e => {
        ...
        if (surplus !== 0) {
           ...
           * 设置理想移动的距离
           */
          this.$set(distance, index, tarDir)
          /**
           * 开启过渡效果
           */
          this.openTransition(400)
  ...
  created() {
     ...
     * 生成数组，赋值到列表
     */
    this.listData[0].list = Array.from({ length: max - min }, (k, v) => min + v)
    let arr = []
    /**
     * 判断 type 值,即生成可显示列表 id 值
     */
    switch (type) {
      case 'year-month':
        arr = [1, 2]
     ...
     * 需要创建数组的数量
     */
    const lenObj = { length: arr.length }
    /**
     * 滚动的距离
     */
    this.distance = Array.from(lenObj, () => 88)
    /**
     * 当前值
     */
    this.currentValue = Array.from(lenObj)
    arr.forEach((elem, i) => {
      /**
       * 获取 list
       */
      const list = listData[elem - 1].list
      /**
       * 判断是否为数值
       */
      this.currentValue[i] = typeof list === 'number' ? +!(i === 3 || i === 4) : list[0]
    })
  }
}
</script>
