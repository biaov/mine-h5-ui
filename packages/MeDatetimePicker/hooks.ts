...
export const useHandMove = (props: Props) => {
  ...
  const ids = typeIds[props.type] ?? [1, 2, 3]; // type值,即生成可显示列表id值
  const show = ref<number[]>(ids); // 当前可显示列
  const lenObj = { length: ids.length }; // 需要创建数组的数量
  const distance = ref<number[]>(Array.from(lenObj, () => 88)); // 滚动的距离
  const currentValue: number[] = Array.from(lenObj); // 当前value值
  ids.forEach((elem, i) => {
  ...
  const setDateDist = (i: number) => {
    ...
    let oldDist = distance.value[index]; // 实际移动的距离
    const len = (listData.value[2].list as number) - 1; // 获取天个数
  ...
  const setValue = (tarDir: number, i: number) => {
    ...
    currentValue[show.value.indexOf(i)] = current; // 设置currentValue
  };
  // 触摸开始
  ...
  const onTouchend = (e: TouchEvent, i: number) => {
    ...
    if (surplus !== 0) {
      ...
      const { list } = listData.value[i - 1]; // 获取当前列项的列表内容
...
// 按钮
export const useBtns = (props: Props, currentValue: number[]) => {
